...
\begin{proof}
    First observe that, by~\Cref{obs:monotone}, $L(S\cup\{v\})-L(S)\geq 0$, meaning this claim is trivial if $L(T\cup\{v\})-L(T)\leq 0$. So we only consider the case where $L(T\cup\{v\})-L(T)>0$, or in other words, there is some portion of the boundary of the polygon seen by the set of guards $T\cup\{v\}$, but not $T$. Then for every point $p$ on this boundary, $p$ is seen by $v$, but not $T$. As $S\subseteq T$, by~\Cref{obs:monotone}, $p$ is not seen by $S$ either. \\\\
